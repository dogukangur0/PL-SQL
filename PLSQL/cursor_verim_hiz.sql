...
DECLARE
    i number := 0;
BEGIN 
    -- cursor kullanmadan, cursor içersine select iþlemi atamadan for içerisinde select iþlemi yapmak verim ve hýz açýsýndan daha iyidir.
    FOR r_c_emp IN (SELECT * FROM EMPLOYEES ORDER BY employee_id) LOOP
        i := i+1;
        dbms_output.put_line(i || ' Employee ID: '|| r_c_emp.employee_id || ' ' || 'Last Name: ' || r_c_emp.last_name);
    END LOOP;
